...
| INFRA-LOCK-001 | Instalações pnpm reproduzíveis com lockfile congelado | MERGED / CI GREEN / DEPLOY PROD EM VALIDAÇÃO | disciplina de build reproduzível | `pnpm-lock.yaml`, `.github/workflows/ci.yml` | PR #198; CI #522 SUCCESS com `--frozen-lockfile`; merge `8056f584`; Railway deploy `7dd2a9c8...` confirmou `Lockfile is up to date` e frozen install; readiness final ainda deve ser confirmado antes de Production-DONE |
| API-001 | API NestJS + Fastify com health/readiness | DEPLOYED / PUBLIC PRODUCTION TRUTH PASS | Documento da Verdade v1.5 §22.3 | `apps/api/` | domínio oficial HTTPS + readiness público já comprovados; novo deploy `7dd2a9c8...` em validação após lockfile |
...
| CI-001 | CI executa typecheck, build, Expo Android export, testes, migration runner, jornadas HTTP e Production Truth contract | ATIVO / GREEN / FROZEN LOCK ENFORCED | Documento da Verdade v1.5 §22.9 | `.github/workflows/ci.yml`, `scripts/http-journey-e2e.sh`, `scripts/http-replacement-e2e.sh`, `scripts/http-notifications-multi-company-e2e.sh` | CI #510/#512/#514/#516/#520/#522 SUCCESS; desde #516 Android export real é obrigatório; desde #522 instalação usa `--frozen-lockfile` |
...
| MULTI-NOTIF-001 | Profissional recebe notificações de múltiplas empresas sem trocar workspace | IMPLEMENTADO / HTTP E2E GREEN / DEPLOYED | tenant isolation + friction gate | `notifications.controller.ts`, tela de notificações mobile | PR #193; CI #510 SUCCESS; merge `1f2b8b53`; deploy `63825398...` SUCCESS; unread agregado e mark-read permanece tenant-scoped |
...
| COMPANY-NAV-001 | Ações principais da empresa acessíveis diretamente pela tela Operação | IMPLEMENTADO / CI GREEN / MOBILE MERGED | Friction Gate | `apps/mobile/app/empresa-inicio.tsx` | PR #186; CI #486 SUCCESS; atalhos para publicar, interessados, substituições, talentos e pagamentos |
| TALENT-POOL-001 | Empresa transforma bom histórico em pool reutilizável e gerencia-o sem UUID técnico | IMPLEMENTADO / HTTP E2E GREEN / MOBILE UX GREEN | Friction Gate + replacement sourcing | `talent-pools.controller.ts`, `empresa-inicio.tsx`, `talentos.tsx` | PR #189 + #194; CI #495/#512 SUCCESS; adicionar a preferred em contexto conhecido, listar por nome e remover em 1 ação |
| REPLACEMENT-001 | Empresa solicita, encontra e confirma substituto sem UUID/manual DB | IMPLEMENTADO / DEDICATED HTTP E2E GREEN / DEPLOYED | Replacement Engine + Trust gate | `replacement.controller.ts`, `substituicoes.tsx`, `http-replacement-e2e.sh` | PR #187–#190; CI #493/#495/#497 SUCCESS; merge #190 `273f730c`; original cancelado e substituto confirmado; sem inferência automática de culpa |
...
|---|---|---|---|---|
| MOB-001 | Aplicativo mobile é o produto operacional principal | IN PROGRESS / ANDROID BUNDLE GREEN / DEVICE E2E PENDENTE | `apps/mobile/` | jornadas HTTP completas verdes; Expo Android export real CI green; device E2E físico pendente |
| MOB-002 | Navegação profissional Início / Trabalhos / Ganhos / Perfil | BASELINE IMPLEMENTADA / DEVICE E2E PENDENTE | `apps/mobile/app/` | telas/rotas presentes; entrada agora protegida por sessão |
...
| MOB-008 | Conversa associada ao assignment + notificações tenant-safe | IMPLEMENTADA / TENANT-ISOLATED / MULTIEMPRESA / HTTP E2E GREEN / DEPLOYED | `conversa.tsx`, `conversations.controller.ts`, `notifications.controller.ts` | PR #183 + #193; CI #478/#510; empresa B recebe 404 ao tentar conversa da empresa A; notificações agregadas só por memberships válidos |
...
| MOB-011 | Entrada/cadastro/logout respeitam sessão segura e tipo de conta | IMPLEMENTADO / CI + ANDROID EXPORT GREEN | `index.tsx`, `entrar.tsx`, `criar-conta.tsx`, `perfil.tsx`, `empresa-inicio.tsx`, `lib/session.ts` | PR #197; CI #520 SUCCESS; merge `67340a0b`; sem sessão → Entrar/Criar conta; empresa/profissional roteados corretamente; logout revoga backend e limpa SecureStore |
| MOB-012 | App em aparelho usa API oficial por padrão sem localhost | IMPLEMENTADO / CI GREEN | `apps/mobile/lib/api.ts` | PR #195; CI #514 SUCCESS; merge `5c510ec6`; `EXPO_PUBLIC_API_URL` continua como override |
| MOB-013 | Bundle Android real é obrigatório no CI | IMPLEMENTADO / EXPO EXPORT GREEN / DEVICE E2E FÍSICO PENDENTE | `apps/mobile/package.json`, `.github/workflows/ci.yml`, `react-native-screens ~4.16.0` | PR #196; CI #516 SUCCESS; merge `cf7f7c3e`; substitui #192 falho sem reduzir o gate |

## Testes integrados da jornada

| ID | Requisito | Estado | Código | Teste/Evidência |
|---|---|---|---|---|
| TEST-JOURNEY-001 | Signup → disponibilidade → vaga → interesse → recomendação → confirmação → assignment → chat/notificação → check-in → início → check-out → conclusão → earnings → avaliação → Work Passport → signout | DB + HTTP E2E GREEN / DEVICE E2E PENDENTE | `professional-journey.sh`, `http-journey-e2e.sh` | multiempresa, tenant isolation, chat, vagas, replacement e Talent Pool; permanece verde até CI #522 |
| TEST-NOTIF-001 | Um profissional + duas empresas → duas confirmações → inbox agregada → mark-read no tenant correto | HTTP E2E GREEN / DEPLOYED | `scripts/http-notifications-multi-company-e2e.sh` | PR #193; CI #510 e regressão green nos CIs seguintes |
| TEST-REPLACEMENT-001 | Dois profissionais → pool preferido → replacement request → auto-match → select → original cancelled → substituto confirmed | HTTP E2E GREEN / DEPLOYED | `scripts/http-replacement-e2e.sh` | PR #190; CI #497 SUCCESS e regressão green até #522; sem fixture direta de negócio |
...
| TEST-COMPANY-ONBOARD-001 | Signup empresa → workspace owner → signin → publicação de vaga válida sem DB fixture | HTTP E2E GREEN | `http-company-onboarding-e2e.sh` | PR #179 + PR #185; permanece green nos CIs recentes |
...
| SEC-POOL-001 | Talent Pools valida papel, histórico e tenant explícito | MERGED / HTTP E2E GREEN / DEPLOYED | `talent-pools.controller.ts` | PR #146/#189; CI #495; UX mobile gerenciada no #194 sem alterar autorização |
...
| SEC-CHAT-001 | Chat/notificações respeitam assignment e tenant explícitos | MERGED / HTTP E2E GREEN / DEPLOYED | `conversations.controller.ts`, `notifications.controller.ts` | PR #183/#193; CI #478/#510; agregação multiempresa limitada às memberships autenticadas |
...
- último deploy de API comprovadamente SUCCESS antes do lockfile: `63825398-c004-4016-ad4e-31c61f6f62a9`, após PR #193, readiness 200;
- deploy após PR #198: `7dd2a9c8-c22e-40da-b234-058fdff872a0` — **EM VALIDAÇÃO** nesta reconciliação; build já comprovou `pnpm install --frozen-lockfile --prefer-offline`, lockfile atualizado e API TypeScript build concluído;
- Postgres Railway: **SUCCESS**;
- migration runner em produção: migrations até `0027_runtime_rls_role.sql`; nova migration não foi adicionada em #193–#198;
- CI #522: **SUCCESS**, incluindo frozen lockfile, Expo Android export, testes, migrations, jornadas HTTP e Production Truth contract;
- verificação externa HTTPS do domínio oficial já está registrada como **PASS**;
- Production Truth Gate público health/readiness permanece **PASS** no último deploy concluído; o deploy `7dd2...` só será marcado Production-DONE após readiness 200;
- Neon não faz parte da arquitetura operacional.

Evidências principais: `docs/evidencias/PRODUCTION_TRUTH_PUBLIC_PASS_2026-09-23.md`, `NETWORK_SHARED_MARKETPLACE_v1.54.md`, `PROFESSIONAL_MULTI_COMPANY_v1.55.md`, `RUNTIME_RLS_ROLE_v1.56.md`, `CHAT_NOTIFICATIONS_TENANT_E2E_v1.57.md`, `COMPANY_JOB_SELECTION_v1.58.md`, `COMPANY_SHIFT_WINDOW_v1.59.md`, `COMPANY_OPERATION_NAVIGATION_v1.60.md`, `REPLACEMENT_OPERATION_v1.64.md`, `MOBILE_AUTH_ENTRY_v1.68.md`, `PNPM_LOCKFILE_v1.69.md`.
