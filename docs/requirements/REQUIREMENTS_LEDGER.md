...
| CI-001 | CI executa typecheck, build, testes, migration runner, jornada HTTP e Production Truth contract | ATIVO / GREEN | Documento da Verdade v1.5 §22.9 | `.github/workflows/ci.yml`, `scripts/http-journey-e2e.sh` | runs #388/#389/#393/#414/#424/#426/#433/#439/#446/#450/#456 SUCCESS; PR #176 merge `785ad0c` adiciona jornada HTTP real |
...
| ONBOARD-001 | Signup/onboarding cria vínculo de tenant/membership sem fixture administrativa | OPEN / GAP CONFIRMADO PELO HTTP E2E | jornada de onboarding | signup/signin existem; criação de tenant/membership ainda não está exposta como fluxo público seguro | PR #176 usa apenas tenant+memberships como fixture mínima; demais passos passam pela API real |
...
| MOB-003 | Interesse em oportunidade em 1 ação | API + MOBILE IMPLEMENTADOS / HTTP+DB INTEGRATION GREEN / DEVICE E2E PENDENTE | Friction Gate | `apps/mobile/app/trabalhos.tsx`, `apps/api/src/jobs.controller.ts`, `job_interests` | PR #156 SQL integration; PR #176 HTTP E2E; CI #456 SUCCESS |
| MOB-004 | Ganhos acessíveis em 1 ação | API + MOBILE BASELINE / HTTP+LEDGER INTEGRATION GREEN / PSP REAL PENDENTE | Friction Gate | `apps/mobile/app/ganhos.tsx`, `apps/api/src/earnings.controller.ts`, `earnings_ledger` | PR #156 valida ledger; PR #176 lê earnings via HTTP; PSP/payout real pendente |
| MOB-005 | Perfil profissional mobile persistente | API + MOBILE IMPLEMENTADOS / ROLE DATA + HTTP E2E GREEN / DEVICE E2E PENDENTE | Jornada profissional | `apps/mobile/app/perfil.tsx`, `apps/api/src/professional-profile.controller.ts` | PR #174 role data; PR #176 profile via HTTP; CI #456 SUCCESS |
| MOB-006 | Próximo trabalho/agenda mobile | ASSIGNMENT LIFECYCLE IMPLEMENTADO / UNIT + DB + HTTP E2E GREEN / DEVICE E2E PENDENTE | Jornada profissional | `apps/mobile/app/agenda.tsx`, `apps/api/src/work-assignments.controller.ts`, `apps/api/src/assignment-lifecycle.ts` | PR #154 transições; PR #156 SQL; PR #176 HTTP; CI #456 SUCCESS |
| MOB-007 | Confirmação pela empresa após interesse | IMPLEMENTADA / POLICY + HTTP E2E GREEN / DEVICE E2E PENDENTE | Jornada empresa/profissional | `apps/api/src/company-jobs.controller.ts`, `apps/api/src/company-confirmation-policy.ts` | PR #152/#157; PR #176 confirmação real via HTTP; CI #456 SUCCESS |
| MOB-008 | Conversa associada ao assignment | BASELINE IMPLEMENTADA / E2E PENDENTE | Jornada profissional | `apps/mobile/app/conversa.tsx`, `apps/api/src/conversations.controller.ts` | autorização por assignment implementada; E2E específico de chat pendente |
| MOB-009 | Empresa avalia profissional após trabalho concluído | IMPLEMENTADA / DB + HTTP INTEGRATION GREEN / DEPLOYED / DEVICE E2E PENDENTE | Jornada empresa → avaliação → Work Passport | `apps/mobile/app/empresa-inicio.tsx`, `apps/api/src/company-dashboard.controller.ts`, `apps/api/src/ratings.controller.ts` | PR #172; PR #176 avaliação e Work Passport via HTTP; CI #456 SUCCESS; deploy do produto `a8f78439...` SUCCESS |

## Testes integrados da jornada

| ID | Requisito | Estado | Decisão/ADR | Código | Teste/Evidência |
|---|---|---|---|---|---|
| TEST-JOURNEY-001 | Signup → disponibilidade → vaga → interesse → recomendação → confirmação → assignment → check-in → início → check-out → conclusão → earnings → avaliação → Work Passport → signout | DB + HTTP E2E GREEN / DEVICE E2E PENDENTE | Plano Mestre / Definition of Done | `packages/db/tests/professional-journey.sh`, `scripts/http-journey-e2e.sh` | PR #156/#172 SQL; PR #176 CI #456 SUCCESS, merge `785ad0c`; HTTP test usa DB separado migrado pelo runner oficial e somente tenant/memberships como fixture direta |
...
| SEC-AVAIL-001 | Availability rejeita timestamps inválidos | MERGED / CI GREEN | validação de entrada | `apps/api/src/availability.controller.ts` | PR #151; CI SUCCESS; PR #176 exercita availability via HTTP |
...
- deployment atual do produto após #175: `d5fb0361-ac9f-49b9-a694-88e97d069b62` — **SUCCESS**;
...
- PR #176 altera apenas testes/CI/evidência e não exige mudança de schema/runtime do produto;
...
- Production Truth contract também passa dentro do CI sobre banco isolado/migrado no PR #176 CI #456;
...
Evidências: `docs/evidencias/DEPLOY_PRODUCTION_BOOTSTRAP_2026-09-22.md`, `docs/evidencias/PRODUCTION_TRUTH_GATE_v1.44.md`, `docs/evidencias/TRUST_VERIFICATIONS_v1.47.md`, `docs/evidencias/COMPANY_RATING_MOBILE_v1.48.md`, `docs/evidencias/MATCH_ROLE_FIT_v1.50.md`, `docs/evidencias/MATCH_CITY_PROXIMITY_v1.51.md` e `docs/evidencias/HTTP_JOURNEY_E2E_v1.52.md`.
