# MLIVRETRABALHO — DOCUMENTO DA VERDADE v1.5
...
> Esta versão consolida integralmente o estado normativo recuperável e incorpora o fechamento do ADR-MT-001. Nenhuma decisão material deve existir somente fora do GitHub.
...
Não inventar arquitetura privada de concorrente. Para arquitetura técnica privada não publicamente documentada, usar fontes técnicas primárias verificáveis e registrar a limitação em vez de inferir implementação de concorrentes.
...
- **ARCH-PG-RLS-001** — PostgreSQL / Row-Level Security.
- **ARCH-AWS-MT-001** — AWS / modelos PostgreSQL multi-tenant.
- **ARCH-AWS-ISO-001** — AWS / tenant isolation.

O registro detalhado deve permanecer em `docs/evidencias/`; uma decisão material baseada em concorrente ou claim técnico não fecha sem evidência verificável.
...
## 23. Multi-tenancy — ADR-MT-001 fechado

**ADR-MT-001 está CLOSED / APROVADO em 20/09/2026.**

Baseline: **modelo pool híbrido controlado**, com schema compartilhado e `tenant_id` + PostgreSQL Row-Level Security obrigatório para dados tenant-owned.

Regras normativas:

1. Toda tabela sensível deve declarar escopo: `TENANT`, `PROFESSIONAL_OWNED`, `NETWORK_SHARED` ou `PLATFORM`.
2. Tabelas `TENANT` usam `tenant_id NOT NULL`, RLS e `FORCE ROW LEVEL SECURITY`.
3. A role runtime da aplicação não pode ser owner, superuser nem possuir `BYPASSRLS`.
4. Contexto de tenant deve ser resolvido a partir da sessão/membership validada e aplicado por transação; o cliente não escolhe livremente o tenant confiável por header/body.
5. Autorização de aplicação (RBAC/ABAC) permanece obrigatória e complementa RLS; não a substitui.
6. Ausência de contexto deve falhar/default-deny para dados tenant-owned.
7. Acesso cross-tenant legítimo deve ocorrer somente por caminho privilegiado explícito, de menor privilégio e auditado.
8. Connection pooling precisa de teste específico contra vazamento de contexto entre requisições.
9. Testes negativos cross-tenant de SELECT/INSERT/UPDATE/DELETE são obrigatórios em CI com PostgreSQL real/compatível.
10. Schema-per-tenant e banco/infra dedicada permanecem opções futuras para requisitos enterprise/regulados; não são defaults.

O ADR completo está em `docs/adr/ADR-MT-001.md`.

**SEC-001 — tenant isolation tem arquitetura definida, mas só fica DONE após implementação, testes reproduzíveis e evidência.** Falha de isolamento é Stop-the-Line.
...
- SEC-001 — tenant isolation (`tenant_id` + RLS baseline; implementação/evidência pendentes).
...
- **ADR-MT-001: CLOSED/APROVADO** — schema de tenancy pode avançar conforme o ADR; SEC-001 continua pendente de implementação/testes/evidência.
...
- Markdown resumido inicial do GitHub declarado insuficiente e substituído por versão integral recuperável;
- Reliability Score passa a exigir causalidade explícita;
- fechamento/reabertura de gates explicitado.

## v1.5
- ADR-MT-001 fechado e documentado;
- baseline de multi-tenancy definido como pool híbrido controlado com `tenant_id` + PostgreSQL RLS para dados tenant-owned;
- `FORCE ROW LEVEL SECURITY`, role runtime sem owner/superuser/BYPASSRLS e contexto por transação tornados requisitos;
- classificação de tabelas em `TENANT`, `PROFESSIONAL_OWNED`, `NETWORK_SHARED` e `PLATFORM`;
- testes negativos cross-tenant e teste de connection-pool leakage tornados obrigatórios;
- schema-per-tenant e tenant dedicado mantidos como extensões futuras, não defaults;
- SEC-001 permanece pendente de implementação, testes e evidência, embora o gate arquitetural ADR-MT-001 esteja fechado;
- Evidence Registry recebe ARCH-PG-RLS-001, ARCH-AWS-MT-001 e ARCH-AWS-ISO-001.
...
| ARCH-PG-RLS-001 | PostgreSQL | Row-Level Security | Verificado em 20/09/2026 |
| ARCH-AWS-MT-001 | AWS Prescriptive Guidance | modelos multi-tenant PostgreSQL | Verificado em 20/09/2026 |
| ARCH-AWS-ISO-001 | AWS SaaS Architecture Fundamentals | tenant isolation | Verificado em 20/09/2026 |
