...
- **Documento da Verdade vigente:** `docs/documento-da-verdade/DOCUMENTO_DA_VERDADE_v1.12.md`, que incorpora as versões anteriores por referência e registra a nova estratégia LEGAL-ARCH.
...
Baseline: split por PSP/provider contratado, provider-neutral no domínio. Escrow, pagamento off-platform, PIX manual por conta operacional e guarantee/advance/credit não são o padrão inicial. PSP real não é autoridade sobre IDs internos; referências externas devem ser resolvidas por binding controlado pelo backend antes de entrar no contexto tenant/RLS. FIN-RISK continua OPEN até provider/contrato/sandbox/compliance e unit economics reais. Parecer jurídico externo não é mais gate obrigatório.

## Trust/KYC baseline
Provider externo de KYC/KYB é autoridade apenas sobre fatos/referências próprios. Callback não pode impor tenant/identity IDs internos, score, assignment/payment action, suspensão, deactivation ou culpa. Referência externa precisa ser resolvida por binding server-controlled antes do contexto tenant/RLS; enforcement material continua separado e sujeito a human review/appeal. TRUST-ARCH continua OPEN até provider real, sandbox, LGPD operacional e pentest. Parecer jurídico externo não é mais gate obrigatório.

## Legal baseline
O projeto segue revisão jurídica interna contínua baseada em fontes primárias, legislação, jurisprudência e melhores práticas. `LEGAL_BEST_PRACTICES_REVIEW_v1.12.md` é o baseline. Essa revisão não equivale a parecer jurídico profissional e não elimina risco residual.
...
## Gates de produção
Production Truth/CI, FIN-RISK, TRUST-ARCH, device/pilot/pentest e WEB-ARCH permanecem separados. LEGAL-ARCH agora é fechado por revisão interna documentada e guardrails rastreados, não por parecer externo obrigatório. `DEPLOYED` isolado não significa Production-DONE.
