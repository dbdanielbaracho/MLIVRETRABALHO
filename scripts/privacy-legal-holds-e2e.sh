...
if PRIVACY_OPERATOR_ID="$OPERATOR_ID" node apps/api/dist/privacy-legal-holds-ops.js create assignment "$ASSIGNMENT" reason evidence >/tmp/legal-hold-no-db.log 2>&1; then
...
if PRIVACY_MAINTENANCE_DATABASE_URL="$DB_URL" node apps/api/dist/privacy-legal-holds-ops.js create assignment "$ASSIGNMENT" reason evidence >/tmp/legal-hold-no-operator.log 2>&1; then
...
HOLD_JSON="$(PRIVACY_MAINTENANCE_DATABASE_URL="$DB_URL" PRIVACY_OPERATOR_ID="$OPERATOR_ID" node apps/api/dist/privacy-legal-holds-ops.js create assignment "$ASSIGNMENT" 'E2E legal hold' 'test:legal-hold')"
HOLD_ID="$(printf '%s' "$HOLD_JSON" | json_field id)"
node -e 'const x=JSON.parse(process.argv[1]);if(x.scopeType!=="assignment"||!x.tenantId||x.createdBy!==process.argv[2])throw new Error("hold audit missing")' "$HOLD_JSON" "$OPERATOR_ID"

LIST_JSON="$(PRIVACY_MAINTENANCE_DATABASE_URL="$DB_URL" node apps/api/dist/privacy-legal-holds-ops.js list)"
node -e 'const rows=JSON.parse(process.argv[1]);if(!rows.some(x=>x.id===process.argv[2]&&x.createdBy===process.argv[3]))throw new Error("active hold not listed")' "$LIST_JSON" "$HOLD_ID" "$OPERATOR_ID"
...
echo 'PASS: legal hold lifecycle is privileged, operator-audited, listed, and releasable'
