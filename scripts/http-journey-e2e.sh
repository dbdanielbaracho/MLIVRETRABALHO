...
COMPANY_SIGNUP="$(request POST /v1/auth/signup '' '' "{\"email\":\"$COMPANY_EMAIL\",\"password\":\"$PASSWORD\",\"accountType\":\"company\",\"workspaceName\":\"HTTP Journey Company\"}")"
...
INTEREST="$(request POST "/v1/jobs/$JOB_ID/interest" "$PRO_TOKEN")"
...
printf '%s' "$PRO_ME" | grep -q 'professional'
...
echo "PASS: HTTP journey no-fixture signup→network availability/job/interest→confirm→membership→work→earnings→rating→passport"
