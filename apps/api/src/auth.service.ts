...
export class AuthService {
 ...
 async identityFromAuthorization(value?:string){
  ...
  const r=await this.db.query<{id:string;email:string}>('SELECT i.id,i.email FROM sessions s JOIN identities i ON i.id=s.identity_id WHERE s.token_hash=$1 AND s.expires_at>now() AND i.deactivated_at IS NULL',[this.hash(token)]);
