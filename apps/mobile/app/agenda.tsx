...
export default function Agenda(){
  ...
  return <SafeAreaView style={s.screen}><View style={s.content}><ProfessionalNav/><Text style={s.title}>Minha agenda</Text>{items.length===0?<Text style={s.empty}>Quando uma oportunidade for confirmada, ela aparecerá aqui.</Text>:items.map(assignment=><View key={assignment.id} style={s.card}><Text style={s.role}>{assignment.title}</Text><Text>{assignment.location??'Local a confirmar'}</Text><Text>Status: {assignment.status}</Text><Pressable onPress={()=>router.push({pathname:'/conversa',params:{assignmentId:assignment.id,tenantId:assignment.tenantId}})}><Text style={s.action}>Abrir conversa</Text></Pressable><Pressable onPress={()=>router.push({pathname:'/seguranca',params:{assignmentId:assignment.id,tenantId:assignment.tenantId}})}><Text style={s.action}>Relatar problema de segurança</Text></Pressable>{assignment.status!=='completed'?<Pressable onPress={()=>void action(assignment)}><Text style={s.action}>{assignment.status==='confirmed'?'Fazer check-in':assignment.status==='checked_in'?'Iniciar trabalho':assignment.status==='in_progress'?'Fazer check-out':'Concluir trabalho'}</Text></Pressable>:<View style={s.ratingBlock}><Text style={s.ratingTitle}>{assignment.companyRatingScore?`Sua avaliação da empresa: ${assignment.companyRatingScore} ★`:'Avalie a empresa'}</Text><View style={s.ratingRow}>{[1,2,3,4,5].map(score=><Pressable key={score} style={s.ratingButton} onPress={()=>void rateCompany(assignment,score)}><Text style={s.ratingText}>{score} ★</Text></Pressable>)}</View></View>}</View>)}<Text>{message}</Text></View></SafeAreaView>;
}
