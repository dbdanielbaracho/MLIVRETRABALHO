...
type Assignment = { id:string; tenantId:string; status:string; title:string; location?:string; startsAt?:string; payCents?:number };
...
export default function Agenda(){
  ...
  return <SafeAreaView style={s.screen}><View style={s.content}><ProfessionalNav/><Text style={s.title}>Minha agenda</Text>{items.length===0?<Text style={s.empty}>Quando uma oportunidade for confirmada, ela aparecerá aqui.</Text>:items.map(assignment=><View key={assignment.id} style={s.card}><Text style={s.role}>{assignment.title}</Text><Text>{assignment.location??'Local a confirmar'}</Text><Text>Status: {assignment.status}</Text><Pressable onPress={()=>router.push({pathname:'/conversa',params:{assignmentId:assignment.id,tenantId:assignment.tenantId}})}><Text style={s.action}>Abrir conversa</Text></Pressable>{assignment.status!=='completed'&&<Pressable onPress={()=>void action(assignment)}><Text style={s.action}>{assignment.status==='confirmed'?'Fazer check-in':assignment.status==='checked_in'?'Iniciar trabalho':assignment.status==='in_progress'?'Fazer check-out':'Concluir trabalho'}</Text></Pressable>}</View>)}<Text>{message}</Text></View></SafeAreaView>;
}
const s=StyleSheet.create({screen:{flex:1,backgroundColor:'#fff'},content:{padding:24,gap:16},title:{fontSize:30,fontWeight:'800'},empty:{fontSize:17},card:{padding:20,borderWidth:1,borderRadius:16,gap:10},role:{fontSize:22,fontWeight:'800'},action:{fontSize:18,fontWeight:'800'}});
